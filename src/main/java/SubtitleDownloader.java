...
public class SubtitleDownloader {
    ...
    private static boolean getSubtitles(String movieName, String movieFilePath) {
        ...
        if (movieName == null || movieName.isEmpty()) {
            System.out.println("\nPlease enter a movie name to search for subtitles (Press ENTER to skip): ");
            movieName = s.nextLine().trim();
            if (movieName.isEmpty()) return false;
        }
        System.out.println("\nMovie: " + movieName);
        try {
            String langCode = language.equalsIgnoreCase("English") ? "en" : language.substring(0, 2).toLowerCase();
            String fileId = OpenSubtitlesApiHelper.searchSubtitle(movieName, langCode);
            if (fileId == null) {
                System.out.println("No subtitles found for '" + movieName + "' on OpenSubtitles.org");
                return false;
            }
            String outputFolder = movieFilePath != null ? getMovieFolder(movieFilePath) : System.getProperty("user.dir");
            String outputPath = outputFolder + File.separator + movieName.replaceAll("[\\/:*?\"<>|]", "_") + ".srt";
            boolean success = OpenSubtitlesApiHelper.downloadSubtitle(fileId, outputPath);
            if (success) {
                System.out.println("\nSuccessfully downloaded subtitle for '" + movieName + "' to: " + outputPath);
                System.out.println("---------------------------------------------------------------" +
                        "---------------------------------------------------\n");
                return true;
            } else {
                System.out.println("Failed to download subtitle file.");
                return false;
            }
        } catch (Exception e) {
            System.out.println("Exception: " + e);
    ...
    private static String getMovieName(String movieFilePath) {
        ...
        try {
            ...
            Document document = Jsoup.connect(yifySubtitleUrl)
            ...
            if (element != null && "no results".equals(element.text())) {
                ...
                String omdbJsonData = Jsoup.connect(OmdbUrl)
                    .userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36")
                    .header("Accept-Language", "en-US,en;q=0.9")
